...
void locking_state_systrace_c(unsigned int cpu, struct task_struct *p)
{
	...
	/*
 	* 0: ots not alloc，not be protected.
 	* 1: ots alloced but not locking, not be protected.
 	* 2: ots alloced and locking, be protected.
 	* */
	if (IS_ERR_OR_NULL(ots)) {
		...
		if (!oplus_list_empty(&ots->locking_entry))
			locking_state = (ots->locking_start_time > 0 ? 3 : 1);
		else
			locking_state = (ots->locking_start_time > 0 ? 2 : 1);
...
static void android_vh_mutex_wait_start_handler(void *unused, struct mutex *lock)
{
	...
	ots->locking_start_time = jiffies;
}
...
static bool locking_depth_skip(int locking_depth)
{
	/*
